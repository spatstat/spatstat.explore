...
#
#   $Revision: 1.31 $   $Date: 2022/05/23 02:33:06 $
#
...
rho2hat <- function(object, cov1, cov2, ..., method=c("ratio", "reweight")) {
  ...
  if(is.ppp(object) || is.quad(object)) {
    ...
    if(!requireNamespace("spatstat.model"))
      stop("The package spatstat.model is required when 'object' is a fitted model",
           call.=FALSE)
    X <- spatstat.model::data.ppm(model)
    modelcall <- model$call
    if(is.null(spatstat.model::getglmfit(model)))
